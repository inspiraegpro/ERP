    ...
    function createRow(d = {}) {
        ...
        filteredProducts.forEach(p => opts += `<option value="${p._id}" data-price="${p.pricing?.salePrice || p.pricing?.unitSalePrice || 0}" data-unit="${p.unit||'ROLL'}" ${d.product===p._id?'selected':''}>${p.name}</option>`);
        ...
        tr.innerHTML = `
            ...
            <td><input type="number" class="area readonly-input" value="${d.area||0}" readonly oninput="updPrice(this.closest('tr').querySelector('.prod'))"></td>
